 ...
 *
 * <p>Runtime-tunable values (API key, token provider, base URL, timeouts, model,
 * temperature, max tokens) are read from {@link HubSettings} on every call, so
 * they can be updated dynamically without recreating the hub.</p>
 *
...
class SynapseRequestBuilder {
    ...
    /**
     * Constructs a new {@code SynapseRequestBuilder}.
     *
     * @param settings     the runtime-tunable hub settings; must not be {@code null}
     ...
     * @since 1.0.0
     */
    SynapseRequestBuilder(HubSettings settings, ObjectMapper objectMapper, ProviderAdapter adapter) {
     ...
     */
    String buildUrl() {
        return adapter.buildUrl(settings.baseUrl, settings.endpoint);
    }
     ...
     * <ul>
     *   <li>{@code model} - the current default model name</li>
     *   <li>{@code messages} - the provided message list</li>
     *   <li>{@code temperature} - the current default temperature</li>
     *   <li>{@code max_tokens} - the current default max tokens</li>
    ...
    Map<String, Object> buildMessagesBody(List<ChatMessage> messages, boolean stream) {
        return buildMessagesBody(messages, stream, settings.modelName);
    }

    Map<String, Object> buildMessagesBody(List<ChatMessage> messages, boolean stream, String modelName) {
        return adapter.buildChatBody(messages, settings.temperature, settings.maxTokens, modelName, stream);
    }

    Map<String, Object> buildMessagesBody(List<ChatMessage> messages, boolean stream, String modelName,
                                          List<ToolDefinition> tools, String responseFormat) {
        return adapter.buildChatBody(messages, settings.temperature, settings.maxTokens, modelName,
                stream, tools, responseFormat);
    }
     ...
     *   <li>{@code Authorization}: the token provider header if one is configured,
     *       otherwise {@code Bearer <api-key>}</li>
     *   <li>Timeout from the current request timeout setting</li>
    ...
    HttpRequest buildPostRequest(String url, String body) {
        return HttpRequest.newBuilder()
                .uri(URI.create(url))
                .headers(buildHeaderArray(buildHeaders()))
    ...
    private Map<String, String> buildHeaders() {
        TokenProvider tokenProvider = settings.tokenProvider;
        if (tokenProvider != null) {
            Map<String, String> headers = new HashMap<>(
                    adapter.buildHeaders(settings.apiKey != null ? settings.apiKey : ""));
            headers.put("Authorization", tokenProvider.buildAuthorizationHeader());
            return headers;
        }
        return adapter.buildHeaders(settings.apiKey);
    }
